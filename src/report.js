...
class Report {
   ...
   * @param {String} options.name         Committer name
   * @param {String} options.email        Committer email
   ...
   */
  constructor(octokit, {fp, name, email, owner, repo, enterprise = ''}) {
  ...
  async getPendingInvitations(org, invitees) {
    try {
      const invitations = await this.octokit.paginate(this.octokit.orgs.listPendingInvitations, {
        org
      })
  ...
  async getFailedInvitations(org, invitees) {
    try {
      const invitations = await this.octokit.paginate(this.octokit.orgs.listFailedInvitations, {
        org
      })
  ...
  async pushReport(opts) {
    ...
    } = await this.octokit.repos.createOrUpdateFileContents(opts)
